...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(
            tf.int32, shape=[None, self.config.n_features], name='Input')
        self.labels_placeholder = tf.placeholder(
            tf.float32, shape=[None, self.config.n_classes], name='Target')
    ...
    def add_embedding(self):
        ...
        with tf.device('/cpu:0'):
            ...
            embedding = self.pretrained_embeddings
                # tf.get_variable('Embedding', [len(self.wv), self.config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope('Layer1', initializer=xavier_weight_init()) as scope:
            W = tf.get_variable(
                'W', [self.config.n_features * self.config.embed_size,
        ...
        pred = tf.nn.dropout(y, self.dropout_placeholder)
        ### END YOUR CODE
        return pred
    ...
    def add_loss_op(self, pred):
        ...
        cross_entropy = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(pred, self.labels_placeholder))
